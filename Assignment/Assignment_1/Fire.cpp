...
void updateFire(Grid<int>& fire) {
    ...
    for (row = 1; row < fire.numRows(); row++) {
        for (col = 0; col < fire.numCols(); col++) {
            if (fire.inBounds(row - 1, col - 1) && fire.inBounds(row - 1, col)
                && fire.inBounds(row - 1, col + 1)) {
                cellPosition = randomInteger(-1, 1);
            } else if (fire.inBounds(row - 1, col - 1) && fire.inBounds(row - 1, col)) {
                cellPosition = randomInteger(-1, 0);
            } else if (fire.inBounds(row - 1, col + 1) && fire.inBounds(row - 1, col)) {
                cellPosition = randomInteger(0, 1);
            } else {
                cellPosition = 0;
            }

            fire[row - 1][col + cellPosition] = fire[row][col];
            if (fire[row][col] > 0 && randomChance(2.0 / 3.0)) {
                fire[row - 1][col + cellPosition]--;
            }
        }
    }
}
